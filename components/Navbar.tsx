  ...
  const NavLink = ({ to, label, icon: Icon }: { to: string, label: string, icon?: React.ElementType }) => {
    ...
    return (
      <Link to={to} className="group relative flex items-center gap-2 px-1 py-2">
        <span className={`text-sm font-semibold font-opensans tracking-wider transition-colors duration-300 flex items-center gap-2 ${active ? 'text-heritage-green' : 'text-gray-800 hover:text-heritage-green'
          }`}>
          {Icon && <Icon size={18} className={active ? "stroke-[2.5px]" : ""} />}
          {label}
        </span>
        <span className={`absolute bottom-0 left-0 w-full h-0.5 bg-heritage-green transform origin-left transition-transform duration-300 ease-out ${active ? 'scale-x-100' : 'scale-x-0 group-hover:scale-x-100'
          }`} />
      </Link>
    );
  };
    ...
    <nav className="bg-white/95 backdrop-blur-md shadow-sm sticky top-0 z-50 transition-all duration-300 border-b border-gray-100/50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-24">
          <div className="flex items-center gap-6 absolute left-12 top-1/2 transform -translate-y-1/2 z-50">
          ...
          <div className="hidden lg:flex items-center space-x-8 xl:space-x-10 absolute left-1/2 transform -translate-x-1/2">
            ...
            <form onSubmit={handleNavSearch} className="relative group mx-2 translate-y-0">
          ...
          <div className="flex items-center gap-6 absolute right-12 top-1/2 transform -translate-y-1/2 z-50">
      ...
      {isOpen && (
        <div className="lg:hidden bg-white border-t border-gray-100 animate-slideDown shadow-xl">
          <div className="px-4 pt-4 pb-6 space-y-2">
            <Link to="/" onClick={() => setIsOpen(false)} className="block px-4 py-4 rounded-xl text-sm font-semibold font-opensans tracking-wide text-gray-800 hover:text-heritage-green hover:bg-heritage-light transition-all duration-200">Home</Link>
            <Link to="/store" onClick={() => setIsOpen(false)} className="block px-4 py-4 rounded-xl text-sm font-semibold font-opensans tracking-wide text-gray-800 hover:text-heritage-green hover:bg-heritage-light transition-all duration-200">Store</Link>
              ...
              <form onSubmit={(e) => { handleNavSearch(e); setIsOpen(false); }}>
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-white border-2 border-heritage-green text-gray-900 text-sm font-opensans rounded-xl pl-12 pr-4 py-3 outline-none focus:border-heritage-accent hover:border-heritage-accent focus:ring-0 placeholder-gray-400"
                  />
            ...
            <Link to="/assistant" onClick={() => setIsOpen(false)} className="block px-4 py-4 rounded-xl text-sm font-semibold font-opensans tracking-wide text-gray-800 hover:text-heritage-green hover:bg-heritage-light transition-all duration-200">AI Assistant</Link>
            <Link to="/analysis" onClick={() => setIsOpen(false)} className="flex items-center gap-3 px-4 py-4 rounded-xl text-sm font-semibold font-opensans tracking-wide text-gray-800 hover:text-heritage-green hover:bg-heritage-light transition-all duration-200">
              <Camera size={20} /> Analyze Product
            </Link>
            <Link to="/studio" onClick={() => setIsOpen(false)} className="flex items-center gap-3 px-4 py-4 rounded-xl text-sm font-semibold font-opensans tracking-wide text-gray-800 hover:text-heritage-green hover:bg-heritage-light transition-all duration-200">
